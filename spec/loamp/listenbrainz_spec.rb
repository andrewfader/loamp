...
RSpec.describe Loamp::ListenBrainz do
  let(:response) do
    body = [{ 'artist_mbid' => 'related', 'score' => 0.8, 'name' => 'Related' }]
  ...
  it 'normalizes similar artists from the public Labs endpoint' do
    ...
    expect(service.similar_artists('12345678-1234-1234-1234-123456789abc')).to eq(
      [['related', 0.8, 'Related']],
    )
    expect(client).to have_received(:get).with(
      a_string_including('artist_mbid=', 'algorithm='),
    )
    expect(service.similar_artists('not-an-mbid')).to eq([])
  end

  it 'normalizes a single Labs object as well as a list' do
    stub_const('Loamp::Metadata::MBID', /\A[a-f0-9-]{36}\z/i)
    body = { 'artist_mbid' => 'related', 'score' => 0.5, 'name' => 'Related' }
    allow(client).to receive(:get).and_return(
      Loamp::Http::Client::Response.new(status: 200, body: JSON.generate(body)),
    )

    expect(service.similar_artists('12345678-1234-1234-1234-123456789abc'))
      .to eq([['related', 0.5, 'Related']])
  end
